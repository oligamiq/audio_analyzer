pub fn analyzer(wav_file: &mut audio_analyzer_core::prelude::TestData, sample_rate: u32) {
    ...
    loop {
        let abstract_input_node_out_37_0: ndarray::Array1<f64> = wav_file
            .try_recv()
            .unwrap()
            .into_iter()
            .map(|v| v as f64)
        ...
        let tmp_expr_nodes_25 = {
            let is_first_longer =
                cycle_buffer_node_out_4_0.len() as f64 > expr_nodes_out_23_0.len() as f64;
            if is_first_longer {
                cycle_buffer_node_out_4_0
                    .clone()
