...
pub fn analysis(snarl: &Snarl<FlowNodes>) -> anyhow::Result<TokenStream> {
    ...
    if let Some(in_pins) = before_nodes.get(&0) {
        ...
        let mut all_nodes = all_nodes
            .into_iter()
            .filter(|id| *id != abstract_input_node_id)
            .map(|id| {
        ...
        println!(
            ...
            all_nodes
                .iter()
                .map(|(id, _)| node_title(snarl.get_node(*id).unwrap()))
                .collect::<Vec<_>>()
        );
        ...
        loop {
            ...
            assert_eq!(
                outer_code.to_string(),
                proc_macro2::TokenStream::new().to_string()
            );
        }

        let hop_size_ident = gen_node_name_scratch(snarl, &in_pins.first().unwrap().node, 0);

        let sample_rate_ident = gen_node_name_scratch(snarl, &abstract_input_node_id, 1);

        hop_size.extend(quote::quote! {
            {
                let #sample_rate_ident = sample_rate;
